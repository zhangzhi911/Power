...
//上报、驳回的方法	
	public int updateBy(sheet2 s) {
		...
		String sql="UPDATE `11_18brand`.`sheet3` SET `qing` = '"+s.getQing()+"' WHERE `aid` ="+s.getAid();
...
//	修改的方法
	public int updatesheet(sheet2 s) {
		...
		String sql="UPDATE `11_18brand`.`sheet3` SET `fid` = '"+s.getFid()+"' , `fname` = '"+s.getFname()+"' , `pnum` = '"+s.getPnum()+"' , `gong` = '"+s.getGong()+"' , `gprice` = '"+s.getGprice()+"' , `gdian` = '"+s.getGdian()+"' , `gsav` = '"+s.getGsav()+"' , `maxprice` = '"+s.getMaxprice()+"' , `minprice` = '"+s.getMinprice()+"' , `psave` = '"+s.getPsave()+"' , `zprice` = '"+s.getZprice()+"' , `zhuan` = '"+s.getZhuan()+"' , `qing` = '"+s.getQing()+"' WHERE `aid` ="+s.getAid();
		JdbcUtil.executeUpdate(sql);
